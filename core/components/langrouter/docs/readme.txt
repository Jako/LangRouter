...
This plugin is meant to be used with Babel for MODX Revolution. It takes care of
switching contexts, which hold translations, depending on URL requested by
...
- If an URI doesn't contain cultureKey (or one not defined in Babel
  configuration) AND at least one of the client's accepted languages is defined in
  Babel configuration, then the matching context is served.
...
1. Create one context for each language with the later language subfolder name
   as 'context key' and name it with the language name. Normally the context key
   would be equal with the cultureKey of that language, i.e. `en` as 'context key'
   and `English` as 'context name'.
...
3. For each context create a 'site_url' context setting and fill it with the
   following value: `{server_protocol}://{http_host}{base_url}{cultureKey}/`. MODX
...
4. Fill the MODX system setting 'babel.contextDefault' with the context key of
   ...
   LangRouter.
5. In head section of the template insert the following line `<base
   href="[[++site_url]]" />`.
6. Include the static files from the assets folder in your installation with
   ...
   src="[[++assets_url]]images/whatever.jpg" … />`
7. Set the MODX system setting 'link_tag_scheme' to `-1` (URL is relative to
   site_url)

To create these settings easily, you could use the Cross Contexts Settings
extra (https://modx.com/extras/package/crosscontextssettings) available
on MODX Extras.
...
-------------------------|--------------------------------|---------------------
langrouter.debug         | Log debug informations in the  | No
                         | MODX ystem log.                |
langrouter.response_code | Response code for the redirect | HTTP/1.1 301
                         | to the right context, if the   | Moved Permanently
                         | culture key is not set.        |

CAUTION
=======

Please don't activate the 'friendly_urls_strict' MODX system setting, if you use
LangRouter. That could cause nasty redirect loops.
